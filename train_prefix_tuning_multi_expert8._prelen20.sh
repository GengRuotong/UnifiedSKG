...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node 2 train.py \
    --run_name expert8_de_phm32_top2gate_prelen20 \
    --local_rank -1 \
    --seed 3407 \
    --cfg Salesforce/T5_base_prefix_summary_5domains_upsample2_res_expert8_prelen20.cfg \
    --pretrained_model_path pretrained_model/chinese_t5_pegasus_base/ \
    --freeze_plm True \
    --data_folder_path data/sample_datas_wo_prefix \
    --output_dir output/T5_base_prefix_tuning/5domain_expert8_dephm32_top2gate_prelen20 \
    --do_train \
    --do_eval \
    --do_predict \

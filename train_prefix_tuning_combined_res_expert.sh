...
CUDA_VISIBLE_DEVICES=0,1 python -m torch.distributed.launch --nproc_per_node 2 --master_port 1234 train.py \
    --run_name mt_combined_expert \
    --seed 2 \
    --cfg Salesforce/T5_base_prefix_summary_5domains_upsample2_combined_res_expert_perlayer6.cfg \
    --pretrained_model_path pretrained_model/chinese_t5_pegasus_base/ \
    --freeze_plm True \
    --data_folder_path data/sample_datas_wo_prefix \
    --output_dir output/T5_base_prefix_tuning/mt_combined_expert \
    --do_train \
    --do_eval \
    --do_predict \

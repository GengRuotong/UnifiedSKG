""" PyTorch T5 model with prompt tuning. """
import copy
import warnings
...
class T5EncoderStack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.prefix_encoder = nn.Embedding(self.pre_seq_len, config.d_model)
        self.prefix_drop = nn.Dropout(config.prefix_drop)
        #self.prefix_encoder = copy.deepcopy(embed_tokens)
    ...
    def get_prompt_encoder(self, batch_size):
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            prompts_encoder = self.get_prompt_encoder(batch_size=batch_size)
            prompts_encoder = self.prefix_drop(prompts_encoder)
...
class T5DecoderStack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.prefix_drop = nn.Dropout(config.prefix_drop)
        #self.prefix_decoder = copy.deepcopy(embed_tokens)
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if past_key_values_length == 0:
                ...
                prompts_decoder = self.prefix_drop(prompts_decoder)
...
class T5PromptForConditionalGeneration(T5PreTrainedModel):

...
def matvec_product(W: torch.Tensor, x: torch.Tensor,
    ...
    if not kronecker_prod_gate:
        H_cl = kronecker_product_einsum_batched(phm_rule, W)
        H_list = list(H_cl.split(1))
        H_list.append(H_cl.sum(0))
        y_list = []
        for H in H_list:
            y_list.append(torch.matmul(input=x, other=H))
        '''
    ...
    if bias is not None:
        # y += bias
        output = [y_list[i] + bias for i in range(len(y_list))]
    return output
...
class PHMLinearBlock(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # combine_tensor just like [P_k1,P_v1,...,P_kL, P_vL], and PKi = [y1, y2,...y(d + 1)], d = phm_dim
        # reshape combined tensor according to phm_dim(representative perspective)
        # all_combine just like[[P1_k1,P1_v1,...P1_kL, P1_vL],...[Pd_k1,Pd_v1,...Pd_kL, Pd_vL]]
        # result is a list [O1, O2, ...,O(d+1)], O is a concatenation of all layers upcast results
        size = len(combine_tensor[0])
        for j in range(size):
            all_combine.append([])
            for item in combine_tensor:
                all_combine[-1].append(item[j])
            perspec = torch.cat(all_combine[-1], dim=2)
            result.append(perspec)
        '''
